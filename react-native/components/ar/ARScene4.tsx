...
interface ARScene4Props {
    userLocation: Location.LocationObject | null;
    sceneKey: number;
}

// Internal AR Scene Component
function ARScene4Scene({ userLocation, targetLatitude, targetLongitude }: {
    userLocation: Location.LocationObject | null;
        ...
        <ViroARScene>
            ...
            >
                ...
                <Viro3DObject
                    ...
                    onLoadEnd={() => console.log('ARScene4: Bomb loaded at GPS coordinates')}
                    onError={(event) => {
...
export default function ARScene4({ userLocation, sceneKey }: ARScene4Props) {
    ...
    const TARGET_LATITUDE = 50.834000;
    const TARGET_LONGITUDE = 3.266000;
    ...
    useEffect(() => {
        const fetchArtwork = async () => {
            try {
                ...
                // Try to find "Oorlogsmonument Bissegem" with flexible matching
                const artwork = data.data?.find((artwork: any) =>
                    artwork.Name === 'Oorlogsmonument Bissegem' ||
                    artwork.Name === 'Oorlogsmonument' ||
                    artwork.Name?.toLowerCase().includes('bissegem')
                );
                ...
                console.error('ARScene4: Error fetching artwork:', error);
            }
        };
        fetchArtwork();
    }, []);
    ...
    const ARSceneWrapper = () => (
        <ARScene4Scene userLocation={userLocation} targetLatitude={TARGET_LATITUDE} targetLongitude={TARGET_LONGITUDE} />
    );
