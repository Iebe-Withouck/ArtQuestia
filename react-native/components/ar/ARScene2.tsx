...
function ARScene2Scene({ userLocation, targetLatitude, targetLongitude }: {
    userLocation: Location.LocationObject | null;
        ...
        <ViroARScene>
            ...
            >
                ...
                <Viro3DObject
                    ...
                    onLoadEnd={() => console.log('ARScene2: Flag loaded at GPS coordinates')}
                    onError={(event) => {
...
export default function ARScene2({ userLocation, sceneKey }: ARScene2Props) {
    ...
    // Target GPS coordinates for this scene (different from ARScene1)
    const TARGET_LATITUDE = 50.832000;
    const TARGET_LONGITUDE = 3.264000;

    // Fetch artwork data from Strapi - ARScene2 fetches Het Groeningemonument
    useEffect(() => {
        const fetchArtwork = async () => {
            try {
                ...
                if (data.data && data.data.length > 0) {
                    // Find the specific artwork - try different variations
                    const targetArtwork = data.data.find(
                        (artwork: any) =>
                            artwork.Name === 'Het Groeningemonument' ||
                            artwork.Name === 'Groeningemonument' ||
                            artwork.Name?.toLowerCase().includes('groeninge')
                    );
                ...
                <Animated.View style={[styles.bottomMenu, { height: menuHeight, backgroundColor: '#000' }]}>
                    <TouchableOpacity style={styles.toggleButton} onPress={toggleMenu} activeOpacity={0.8}>
                        <Image
                            ...
                            style={[styles.toggleIcon, { transform: [{ rotate: isMenuExpanded ? '0deg' : '180deg' }] }]}
                        />
                    </TouchableOpacity>
                            ...
                            <View style={styles.infoButtonsRow}>
                                <TouchableOpacity style={styles.buttonContainer}>
                                    ...
                                    <View style={styles.button}>
                                        <Text style={[styles.buttonText, { fontFamily: 'LeagueSpartan-regular' }]}>{artwork.Year || 'N/A'}</Text>
                                    </View>
                                </TouchableOpacity>
                                <TouchableOpacity style={styles.buttonContainer}>
                                    <Text style={[styles.buttonIcon, { fontFamily: 'Impact' }]}>Locatie</Text>
                                    <View style={styles.button}>
                                        <Text style={[styles.buttonText, { fontFamily: 'LeagueSpartan-regular' }]}>{calculatedDistance} km</Text>
                                    </View>
                                </TouchableOpacity>
                                <TouchableOpacity style={styles.buttonContainer}>
                                    <Text style={[styles.buttonIcon, { fontFamily: 'Impact' }]}>Thema</Text>
                                    <View style={styles.button}>
                                        <Text style={[styles.buttonText, { fontFamily: 'LeagueSpartan-regular' }]}>{artwork.Theme || 'N/A'}</Text>
                                    </View>
                                </TouchableOpacity>
                            </View>
                            {artwork.Description && (
